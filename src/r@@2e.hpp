...
namespace gui {
  ...
  void winsize_and_realloc(){
    ...
    #define tryalloc(A,B,C,D) DO((!A)&&(A=(B)malloc(C))==NULL)ORDIE("couldn't allocate for " #A);state|=STATE_ ## D;
    tryalloc(term_buffer,char*,max_chars,TBUF);tryalloc(term_buffer,char*,max_chars,TBUF);
